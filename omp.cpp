...
int main(void) {
    unsigned seed = time(0), dim_num = 3, threads = 1;
    ...
    gener rng(1664525, 1013904223, seed, threads);
    omp_set_num_threads(threads);
    unsigned iter_num = 100000000;
    ...
    #pragma omp parallel
   {
    #pragma omp for reduction(+:value)
        for (int i=0; i<iter_num; i++) {
            ...
            for (int j=0; j < dim_num; j++) {
                x[j] = borders[j].first + (borders[j].second - borders[j].first) * rng.next(omp_get_thread_num());
    ...
    std::cout << "Время выполнения программы = " << timerOpenMp << 's' << std::endl;
